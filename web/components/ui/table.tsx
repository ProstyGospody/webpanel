...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-auto rounded-xl border border-border/70 bg-card"
    >
      <table
        data-slot="table"
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b transition-colors hover:bg-muted/35 data-[state=selected]:bg-muted/45",
        className
      )}
      {...props}
    />
